...
const kunRuntimePatchSchema = z.object({
  ...
  webSearch: z.object({
    ...
    fetchProvider: z.string().trim().min(1).max(128).optional(),
    fetchFallbackEnabled: z.boolean().optional(),
    fetchReaderBaseUrl: z.string().trim().max(MAX_URL_LENGTH).optional(),
    fetchApiKey: z.string().max(MAX_BODY_BYTES).optional(),
    apiKey: z.string().max(MAX_BODY_BYTES).optional(),
    baseUrl: z.string().trim().max(MAX_URL_LENGTH).optional(),
...
const scheduledTaskPatchSchema = z.object({
  ...
  prompt: z.string().max(MAX_CHANNEL_TEXT_LENGTH).optional(),
  workspaceRoot: defaultPathSchema,
  taskWorkspaceSourceRoot: defaultPathSchema,
  taskWorkspaceRoot: defaultPathSchema,
  taskWorkspaceKind: z.enum(['git-worktree', 'snapshot', 'workspace']).optional(),
  taskWorkspaceState: z.enum(['idle', 'preparing', 'ready', 'running', 'archived', 'cleaned', 'error']).optional(),
  taskWorkspaceRepoRoot: defaultPathSchema,
  taskWorkspaceRef: z.string().trim().max(MAX_PATH_LENGTH).optional(),
  taskWorkspacePreparedAt: z.string().max(128).optional(),
  taskWorkspaceArchivedAt: z.string().max(128).optional(),
  taskWorkspaceCleanedAt: z.string().max(128).optional(),
  taskWorkspaceError: z.string().max(MAX_CHANNEL_TEXT_LENGTH).optional(),
  taskRuleSources: z.array(z.string().trim().min(1).max(MAX_PATH_LENGTH)).max(128).optional(),
  taskRuleSummary: z.string().max(MAX_CHANNEL_TEXT_LENGTH).optional(),
  taskActiveSkillIds: z.array(z.string().trim().min(1).max(128)).max(64).optional(),
  taskSkillInjectionBytes: z.number().int().nonnegative().optional(),
  model: z.string().trim().min(1).max(128).optional(),
  reasoningEffort: scheduleReasoningEffortSchema.optional(),
  mode: clawRunModeSchema.optional(),
