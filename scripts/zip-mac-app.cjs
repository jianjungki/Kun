...
const version = (process.env.PENGCODEX_APP_VERSION || process.env.DEEPSEEK_GUI_APP_VERSION || pkg.version || '').trim()
if (!version) {
  console.error('[zip-mac-app] Could not resolve package version.')
  process.exit(1)
}

const distDir = resolve(process.env.PENGCODEX_DIST_DIR || process.env.DEEPSEEK_GUI_DIST_DIR || join(root, 'dist'))
const appOutDir = join(distDir, arch === 'arm64' ? 'mac-arm64' : 'mac')
const appName = 'PengCodex.app'
const appPath = join(appOutDir, appName)
const zipPath = join(distDir, `PengCodex-${version}-mac-${arch}.zip`)
